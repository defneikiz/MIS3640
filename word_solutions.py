...
def find_words_using_all_vowels():
    fin= open('session09/words.txt')
    counter = 0
    # counter_total=0 #if you want to count all the words and get a percentage
    for line in fin:
        # counter_total +=1
        word= line.strip()
        if uses_all(word,'aeiou'):
            print(word)
            counter +=1
    return counter #you would adjust here as counter/counter_total
...
# important question type
# Using the previous function, create a new function that counts the abecedarian words in English (words.txt file)
...
print(longest_abecedarian_word())
